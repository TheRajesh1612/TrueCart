...
const Mobiles = () => {
  ...
  const selectedItem = mobileData.find((item) => item.id === showCard);

  return (
    <>
      <div className="w-full h-full ">
        <h3 className="px-4 pt-2 font-serif font-light ">Mobiles</h3>
        ...
        >
          ...
          {mobileData.map((item, index) => (
            <motion.div
              key={item.id}
              className={`flex flex-col  min-w-fit cursor-pointer rounded-xl shadow-3xl ${
                isDarkMode
