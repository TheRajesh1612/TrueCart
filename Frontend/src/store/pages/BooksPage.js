...
const BooksPage = () => {
  ...
  function companyHandler(title) {
    if (selectProduct.includes(title)) {
      setSelectProduct(selectProduct.filter((item) => item !== title));
    } else {
      setSelectProduct([...selectProduct, title]);
    }
  }

  // Filtered books data based on selected companies
  const filteredProduct =
    selectProduct.length === 0
      ? booksData
      : booksData.filter((item) => selectProduct.includes(item.title));
        ...
        <div className=" flex flex-col lg:flex-row">
          ...
          >
            ...
            <div className="   flex flex-col gap-2 p-4  ">
              {booksData.map((item) => (
                <div>
                  <label className=" font-sans cursor-pointer text-sm">
                    <input
                      className="mr-2"
                      type="checkbox"
                      checked={selectProduct.includes(item.title)}
                      onChange={() => {
                        companyHandler(item.title);
                      }}
                    />
                    {item.title}
                  </label>
              ...
              {filteredProduct.map((item, index) => (
                ...
                >
                  ...
                  <div className=" md:w-1/2 p-4 flex flex-col justify-evenly">
                    <h3 className=" text-lg font-sans">{item.title}</h3>
