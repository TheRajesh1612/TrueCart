...
const WomenPage = () => {
  ...
  function companyHandler(brand) {
    if (selectProduct.includes(brand)) {
      setSelectProduct(selectProduct.filter((item) => item !== brand));
    } else {
      setSelectProduct([...selectProduct, brand]);
    }
  }

  // Filtered women data based on selected companies
  const filteredProduct =
    selectProduct.length === 0
      ? womanData
      : womanData.filter((item) => selectProduct.includes(item.brand));
    ...
    <>
      ...
      <div className="  flex flex-col min-h-screen bg-gray-200">
        ...
        <div className=" flex flex-col lg:flex-row">
          ...
          >
            <h2 className=" text-sm font-sans text-decoration-underline py-3 text-gray-500  m-0">
              Explore Our women Collection
            </h2>
            {/* Filter Options */}
                ...
                <div>
                  <label className=" font-sans cursor-pointer">
                    <input
                      className="mr-2"
                      type="checkbox"
                      checked={selectProduct.includes(item.brand)}
                      onChange={() => {
                        companyHandler(item.brand);
                      }}
                    />
                    {item.brand}
                  </label>
              ...
              {filteredProduct.map((item, index) => (
                ...
                >
                    ...
                    <div className=" md:w-1/2 p-4 flex flex-col justify-evenly">
                      <h3 className=" text-lg font-sans">{item.brand}</h3>
                      ...
                      <button className=" bg-orange-500 py-2 font-sans rounded-full text-white text-sm hover:bg-orange-600 transition"
                      onClick={()=>{
                        addToCart(item);
